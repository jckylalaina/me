  ...
  return (
    <div className='-my-6'>
      {timelines?.map((timeline: TimelineProps, index: number) => (
        <div key={index} className={`relative pl-8 sm:pl-14 py-6 group ${timeline.enterpriseURL && 'cursor-pointer' || 'pointer-event-none'}`} onClick={() => handleTimelineClick(timeline.enterpriseURL || "")}>
